...
export default async function handler(req, res) {
  ...
  if (!turma) {
    return res.status(400).json({ mensagem: 'Turma não fornecida' });
  }
  ...
  try {
    ...
    // Busca projetos da turma com suas relações
    const query = `
      SELECT 
        p.id_projeto,
        p.titulo,
      ...
      WHERE p.turma = $1 AND p.ativo = true
      GROUP BY p.id_projeto
      ORDER BY p.data_criacao DESC
    `;

    const result = await client.query(query, [turma]);
